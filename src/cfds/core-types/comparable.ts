import { CoreOptions, CoreType, CoreValue } from './base';
import { encodableValueHash } from '../encoding/hash';
import { getCoreType, isEquatable, isComparable } from './utils';
import { ChecksumEncoderOpts } from '../encoding/checksum';
...
export function coreValueCompare(
  ...
  const hash1 = encodableValueHash(
    v1 as CoreValue,
    options as ChecksumEncoderOpts
  );
  const hash2 = encodableValueHash(
    v2 as CoreValue,
    options as ChecksumEncoderOpts
  );
  if (hash1 > hash2) {
