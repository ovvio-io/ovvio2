...
import { dictEquals, Dictionary } from '../collections/dict';
import {
  CoreKey,
  CoreOptions,
  CoreType,
...
import { getCoreType, isComparable, isEquatable } from './utils';
...
export function coreValueEquals(
  ...
  switch (t1) {
    ...
    case CoreType.Number:
      return Utils.numbersEqual(e1 as number, e2 as number);

    case CoreType.Date:
      return Utils.numbersEqual((e1 as Date).getTime(), (e2 as Date).getTime());
    ...
    case CoreType.Set: {
      ...
      if (Utils.Set.equals(s1, s2, iterableFilter)) {
        return true;
      }
