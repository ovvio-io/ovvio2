 ...
 */
import Utils, { EnvVars } from '@ovvio/base/lib/utils';
import {
  RowData,
  CRUD,
  ScanningCRUD,
...
class TxnRowState {
  ...
  static async lock(
    ...
    Utils.assert(isUpdateSuccessful(result)); // Sanity check
  ...
  setData(data: RowData | undefined, status: TxnRowStatus) {
    ...
    if (data !== undefined) {
      // Sanity check
      Utils.assert(data[ROW_COL_TXN] === this.txnKey);
  ...
  async finalizeDataRowAndUnlock(): Promise<boolean> {
    switch (this._status) {
      ...
      case TxnRowStatus.Replaced:
        Utils.assert(this._data !== undefined); // Sanity check
  ...
  async unlockRow(): Promise<boolean> {
    ...
    Utils.assert(this._status !== TxnRowStatus.Unlocked);
    ...
    Utils.assert(result !== UpdateResult.Created); // Sanity check
  ...
  private async replaceRow(): Promise<boolean> {
    Utils.assert(this._status === TxnRowStatus.Replaced);
    Utils.assert(this._data !== undefined); // Sanity check
...
export class Transaction implements IACIDTransaction {
  ...
  private async init(): Promise<void> {
    ...
    // Sanity check
    Utils.assert(success, 'Attempting to double init a transaction');
    this._status = TxnStatus.New;
  ...
  async cleanup(): Promise<void> {
    ...
    Utils.assert(this._status === TxnStatus.Commit); // Sanity check
...
export async function releaseRowOrThrow(
  ...
  if (txnData[TXN_COL_STATUS] === TxnStatus.New) {
    ...
    Utils.assert(updateResult !== UpdateResult.Created);
    Utils.assert(
