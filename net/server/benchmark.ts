...
const CHUNK_SIZE = 10000; // Adjust this value based on memory constraints
...
export async function runInsertBenchmark(
  ...
  for (let i = 0; i < testSize; i += chunkSize) {
    ...
    for (let j = 0; j < chunkSize && i + j < testSize; ++j) {
  ...
  results.testSize = testSize;
  results.totalInsertTime = testTime;
  results.avgInsertTime = testTime / testSize;
  return results;
}
...
//     // Run the read benchmark after all insertions in this batch are complete
//     console.log(
