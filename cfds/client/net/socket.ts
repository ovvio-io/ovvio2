...
const START_TIME_MS = 2 * 60 * 1000; //2 minutes
const REQ_NORMAL_TIMEOUT_MS = 10000; //EnvVars.getInt('CFDS_SOCKET_TIMEOUT') || 5000;
const REQ_START_TIMEOUT_MS = REQ_NORMAL_TIMEOUT_MS * 1.5;
...
export class Socket {
  ...
  private processPendingResponses(): boolean {
    ...
    const pendingResponses = this.pendingResponses;
    const inFlightMessages = this.inFlightMessages;
    while (i < 100 && pendingResponses.length > 0) {
  ...
  private getRequestTimeout() {
    ...
    if (Date.now() - this._firstRequestTime <= START_TIME_MS) {
